...
int optee_fast_smc(struct thread_smc_args *args)
{
	struct arm_smccc_res res;

	optee_smccc_smc(args->a0, args->a1, args->a2, args->a3, 
			args->a4, args->a5, args->a6, args->a7, &res);
...
extern struct tee_device* teek_tee_device;
extern spinlock_t ldr_spin_lock;
extern spinlock_t ldr_rpc_spin_lock;
extern spinlock_t ldr_lfc_spin_lock;
...
static struct optee *optee_probe(struct device_node *np)
{
	...
	optee->teedev = teedev;
	mxc_cap_tee_device = teedev;
	teek_tee_device = teedev;
	spin_lock_init(&ldr_spin_lock);
	spin_lock_init(&ldr_rpc_spin_lock);
	spin_lock_init(&ldr_lfc_spin_lock);
...
// unsigned char lfc_stack[1024];
unsigned char *lfc_stack;
EXPORT_SYMBOL(lfc_stack);
...
static int __init optee_driver_init(void)
{
	...
	// memset(lfc_stack, 0, 1024);
	lfc_stack = kmalloc(1024, GFP_KERNEL);
	printk("LFC stack: 0x%08x.\n", lfc_stack);
