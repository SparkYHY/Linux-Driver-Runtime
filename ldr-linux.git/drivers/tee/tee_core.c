...
struct tee_device *mxc_cap_tee_device;
struct tee_context *mxc_cap_tee_ctx = NULL;
bool mxc_cap_tee_ctx_initialized = false;

struct tee_context *mxc_teedev_open(struct tee_device *teedev) {
	return teedev_open(teedev);
}

EXPORT_SYMBOL(mxc_cap_tee_device);
EXPORT_SYMBOL(mxc_cap_tee_ctx);
EXPORT_SYMBOL(mxc_cap_tee_ctx_initialized);
EXPORT_SYMBOL(mxc_teedev_open);
