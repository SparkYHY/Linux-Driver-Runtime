...
extern struct tee_context *mxc_teedev_open(struct tee_device *teedev);

void init_mxc_cap_tee_ctx(void) {
	if (mxc_cap_tee_ctx != NULL || mxc_cap_tee_ctx_initialized)
		return;
	
	if (mxc_cap_tee_device == NULL) {
		pr_info("ERROR: mxc_cap_tee_device uninitialized.\n");
	}

	mxc_cap_tee_ctx = mxc_teedev_open(mxc_cap_tee_device);

	if (mxc_cap_tee_ctx == NULL) {
		pr_info("ERROR: mxc_cap_tee_ctx initialization fails.\n");
	}

	mxc_cap_tee_ctx_initialized = true;
}

void *frame_shm_alloc(uint32_t *size, uint32_t *pa) {
	struct tee_shm *shm;
	struct tee_context *ctx = mxc_cap_tee_ctx;

	shm = tee_shm_alloc(ctx, *size, TEE_SHM_MAPPED | TEE_SHM_DMA_BUF);

	pr_debug("shm->size: 0x%08x\n", shm->size);

	*pa = shm->paddr;
	*size = shm->size;

	return (void *)(shm->kaddr);
}
...
static int mxc_allocate_frame_buf(cam_data *cam, int count)
{
	...
	pr_debug("In MVC:mxc_allocate_frame_buf - size=0x%x\n",
		cam->v2f.fmt.pix.sizeimage);
	pr_debug("\ttotal frame buf count: %d\n", count);
	uint32_t sizeimage = PAGE_ALIGN(cam->v2f.fmt.pix.sizeimage);

	/* re-arrange all frame buffers */
	for (i = 0; i < count; i++) {
		cam->frame[i].vaddress =
	...
	for (i = 0; i < count; i++) {
		pr_debug("\tframe %d: pa: 0x%x, va: 0x%x, len: 0x%x\n", 
			i, cam->frame[i].buffer.m.offset, cam->frame[i].vaddress,
			cam->frame[i].buffer.length);
	}
...
static int mxc_streamon(cam_data *cam)
{
	...
	if (cam->enc_update_eba) {
		...
		frame->ipu_buf_num = cam->ping_pong_csi; // mark frame index
		/* frame->buffer.m.offset: DMA buffer pa */
...
extern int ldr_fiq_callback_on;
...
static int mxc_v4l2_g_fmt(cam_data *cam, struct v4l2_format *f)
{
	...
	/* just print some fmt info, no logics */
...
static int mxc_v4l_dqueue(cam_data *cam, struct v4l2_buffer *buf)
{
	...
	pr_debug("cam->enc_counter:%d is happening.\n", cam->enc_counter);
	...
	pr_debug("In MVC:mxc_v4l_dqueue, buf idx: %d, buf pa: 0x%x\n", 
		frame->index, frame->buffer.m.offset);
			    ...
			    unsigned int ioctlnr, void *arg)
{
	...
	case VIDIOC_QUERYBUF: { //
		...
		if (buf->memory & V4L2_MEMORY_MMAP) { // this branch
			memset(buf, 0, sizeof(*buf));
			buf->index = index;
			pr_debug("querybuf #: %d\n", buf->index);
			/* copy cam->frame[i].buffer to passed buffer */
	...
	case VIDIOC_QBUF: { //
		struct v4l2_buffer *buf = arg;
		int index = buf->index;
		pr_debug("   case VIDIOC_QBUF, buf idx: %d, buf pa: 0x%x\n", 
			index, buf->m.offset);
 ...
 */
/* poll(), as a v4l_fops callback, is at the same level as ioctl() */
...
static void camera_callback(u32 mask, void *dev)
{
	...
	// try to get an empty frame from working queue
	if (!list_empty(&cam->working_q)) {
		// pr_debug("%s: frame from working queue\n", __func__);
		...
		// pr_debug("%s: matched frame from working queue\n", __func__);
		...
		if (done_frame->buffer.flags & V4L2_BUF_FLAG_QUEUED) {
			...
			// pr_debug("%s: done frame pa: 0x%x\n", __func__, 
			// 	done_frame->buffer.m.offset);

			/* Wake up the queue */
			cam->enc_counter++;
			// pr_debug("%s: waking up &cam->enc_queue\n", __func__);
			wake_up_interruptible(&cam->enc_queue);
		} else
...
next:
	// try to get an empty frame from readay queue if working queue is empty
	if (!list_empty(&cam->ready_q)) {
		// pr_debug("%s: prepare frame from ready queue\n", __func__);
		...
		// pr_debug("%s: dummy frame to hold data\n", __func__);
		if (cam->enc_update_eba)
			cam->enc_update_eba(
...
static int init_camera_struct(cam_data *cam, struct platform_device *pdev)
{
	...
	*(cam->video_dev) = mxc_v4l_template;
	...
	init_waitqueue_head(&cam->enc_queue); /* init enc wait queue */
	...
	cam->crop_bounds.left = 0;
	cam->crop_bounds.width = 1280;
	cam->crop_bounds.top = 0;
	cam->crop_bounds.height = 720;
	// cam->crop_bounds.left = 0;
	// cam->crop_bounds.width = 1920;
	// cam->crop_bounds.top = 0;
	// cam->crop_bounds.height = 1080;
	...
	pr_debug("ipu_csi_set_window_size/pos() success.\n");
	...
	cam->standard.framelines = 720;
	// cam->standard.framelines = 1080;
	...
	// cam->v2f.fmt.pix.sizeimage = 640 * 480 * 3 / 2;
	// cam->v2f.fmt.pix.bytesperline = 480 * 3 / 2;
	// cam->v2f.fmt.pix.width = 640;
	// cam->v2f.fmt.pix.height = 480;
	cam->v2f.fmt.pix.sizeimage = 1280 * 720 * 3 / 2;
	cam->v2f.fmt.pix.bytesperline = 720 * 3 / 2;
	cam->v2f.fmt.pix.width = 1280;
	cam->v2f.fmt.pix.height = 720;
	// cam->v2f.fmt.pix.sizeimage = 1920 * 1080 * 3 / 2;
	// cam->v2f.fmt.pix.bytesperline = 1080 * 3 / 2;
	// cam->v2f.fmt.pix.width = 1920;
	// cam->v2f.fmt.pix.height = 1080;
	cam->v2f.fmt.pix.pixelformat = V4L2_PIX_FMT_YUV420;
	// cam->win.w.width = 640;
	// cam->win.w.height = 640;
	cam->win.w.width = 1280;
	cam->win.w.height = 1280;
	// cam->win.w.width = 1920;
	// cam->win.w.height = 1920;
	cam->win.w.left = 0;
	cam->win.w.top = 0;
...
static __init int camera_init(void)
{
	...
	init_mxc_cap_tee_ctx();
	pr_debug("mxc capture tee context initialized.\n");
