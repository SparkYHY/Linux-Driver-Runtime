...
static int prp_enc_eba_update(void *private, dma_addr_t eba)
{
	...
	pr_debug("prp_enc_eba_update: eba %x, BUF%d\n", eba, *buffer_num);
	...
	if (err != 0) {
		if (grotation >= IPU_ROTATE_90_RIGHT) {
			...
			ipu_clear_buffer_ready(ipu, CSI_PRP_ENC_MEM,
					       IPU_OUTPUT_BUFFER,
					       *buffer_num);
			pr_debug("ipu_clear_buffer_ready() called.\n");
			// err = ipu_update_channel_buffer(ipu, CSI_PRP_ENC_MEM,
			// 				IPU_OUTPUT_BUFFER,
			// 				*buffer_num,
			// 				eba);
...
static int prp_enc_enabling_tasks(void *private)
{
	...
	if (cam->rotation >= IPU_ROTATE_90_RIGHT) {
		pr_debug("ipu_prp_enc: register IPU_IRQ_PRP_ENC_ROT_OUT_EOF irq handler.\n");
		err = ipu_request_irq(cam->ipu, IPU_IRQ_PRP_ENC_ROT_OUT_EOF,
				      prp_enc_callback, 0, "Mxc Camera", cam);
	} else {
		pr_debug("ipu_prp_enc: register IPU_IRQ_PRP_ENC_OUT_EOF irq handler.\n");
		pr_debug("prp_enc_callback addr: 0x%08x.\n", prp_enc_callback);
...
static int prp_enc_disabling_tasks(void *private)
{
	...
	if (cam->rotation >= IPU_ROTATE_90_RIGHT) { // false
...
static int prp_enc_enable_csi(void *private)
{
	...
	ret = ipu_enable_csi(cam->ipu, cam->csi);
	return ret;
}
