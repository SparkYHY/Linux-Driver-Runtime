...
struct ldr_ctx thermal_ctx;

spinlock_t thermal_ldr_spin_lock;
...
static int prepare_tee_session(struct ldr_ctx *ctx)
{
	...
	res = TEEK_InitializeContext(&thermal_ctx.ctx);
	if (res != TEEK_SUCCESS) {
	...
	res = TEEK_OpenSession(&thermal_ctx.ctx, &thermal_ctx.sess, &uuid,
...
{
	TEEK_CloseSession(&thermal_ctx.sess);
	TEEK_FinalizeContext(&thermal_ctx.ctx);
}
...
static int sw_probe(void) {
	...
	spin_lock_irqsave(&thermal_ldr_spin_lock, lock_flags);
	...
	*/
	...
	op.params[0].value.a = THERMAL_DRIVER_ID;
	op.params[0].value.b = THERMAL_PROBE;

	/* state variable info */
	op.params[1].tmpref.buffer = imx_thermal_data;
	op.params[1].tmpref.size = sizeof(*imx_thermal_data);

	/* register basic driver info to nw LDR framework */
	ldr_nw_driver_register(THERMAL_DRIVER_ID, imx_thermal_data, 
			sizeof(*imx_thermal_data));

	res = TEEK_InvokeCommand(&thermal_ctx.sess, LDR_CMD_SW_DRIVER_PROBE,
				 &op, &origin);
	if (res != TEEK_SUCCESS) {
		pr_err("TEEK_InvokeCommand(thermal_PROBE) failed 0x%x origin 0x%x",
			res, origin);
		return TEEK_ERROR_GENERIC;
	}

	spin_unlock_irqrestore(&thermal_ldr_spin_lock, lock_flags);
...
static int __init thermal_init(void)
{
	...
	spin_lock_init(&thermal_ldr_spin_lock);

	res = platform_driver_register(&imx_thermal);
	if (res) {
		pr_err("imx_thermal register failed\n");
		return -ENODEV;
	}

	res = prepare_tee_session(&thermal_ctx);
	if (res != TEEK_SUCCESS) {
		pr_err("ERROR: when open session with ldr pta.\n");
	} else {
		printk("success: thermal open session with ldr pta.\n");
	}
