...
static struct imx_thermal_data *get_sw_sv(void) {
	return imx_thermal_data;
}
...
int sw_probe(unsigned char *param_buf) {
	...
	/* copy sv from nw */
	imx_thermal_data = ldr_malloc(sizeof(struct imx_thermal_data));
	if (imx_thermal_data == NULL) {
		return -1;
	}
	memcpy(get_sw_sv(), param_buf, sizeof(struct imx_thermal_data));

	/* sv sw-specific initialization */
	imx_thermal_data->socdata = &thermal_imx6q_data;
	imx_thermal_data->irq_enabled = 0;
	register_thermal_device(THERMAL_DRIVER_ID, imx_thermal_data);

	/* mutex initialization */
	memset(&imx_thermal_data->mutex, NULL, sizeof(imx_thermal_data->mutex));
	mutex_init(&imx_thermal_data->mutex);

	/* regmap initialization */
	imx_thermal_data->tempmon = get_device_base(ANALOG_BASE);
...
static int __init thermal_init(void)
{
	/* register driver */
	pr_debug("inside %s.\n", __func__);
	ldr_sw_driver_register(THERMAL_DRIVER_ID, dispatch, get_sw_sv());

	pr_debug("sw imx thermal init() success.\n");
