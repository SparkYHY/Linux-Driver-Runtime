...
struct ldr_ctx isl29023_ctx;

spinlock_t isl29023_ldr_spin_lock;
				    ...
				    const struct i2c_device_id *id)
{
	...
	res = prepare_tee_session(&isl29023_ctx);
	if (res != TEEK_SUCCESS) {
		pr_err("ERROR: when open session with ldr pta.\n");
	} else {
		printk("success: isl29023 open session with ldr pta.\n");
	}
...
static int prepare_tee_session(struct ldr_ctx *ctx)
{
	...
	res = TEEK_InitializeContext(&isl29023_ctx.ctx);
	if (res != TEEK_SUCCESS) {
	...
	res = TEEK_OpenSession(&isl29023_ctx.ctx, &isl29023_ctx.sess, &uuid,
...
{
	TEEK_CloseSession(&isl29023_ctx.sess);
	TEEK_FinalizeContext(&isl29023_ctx.ctx);
}
...
static int sw_probe(void) {
	...
	spin_lock_irqsave(&isl29023_ldr_spin_lock, lock_flags);
	...
	*/
	...
	op.params[0].value.a = ISL29023_DRIVER_ID;
	op.params[0].value.b = ISL29023_PROBE;

	/* state variable info */
	op.params[1].tmpref.buffer = isl29023_sv;
	op.params[1].tmpref.size = sizeof(struct isl29023_data);

	/* nw sv info */
	op.params[3].value.a = virt_to_phys(isl29023_sv);

	/* register basic driver info to nw LDR framework */
	ldr_nw_driver_register(ISL29023_DRIVER_ID, isl29023_sv, 
			sizeof(struct isl29023_data));

	res = TEEK_InvokeCommand(&isl29023_ctx.sess, LDR_CMD_SW_DRIVER_PROBE,
				 &op, &origin);
	if (res != TEEK_SUCCESS) {
		pr_err("TEEK_InvokeCommand(ISL29023_PROBE) failed 0x%x origin 0x%x",
			res, origin);
		return TEEK_ERROR_GENERIC;
	}

	spin_unlock_irqrestore(&isl29023_ldr_spin_lock, lock_flags);
...
static int __init isl29023_init(void)
{
	...
	res = i2c_add_driver(&isl29023_driver);

	spin_lock_init(&isl29023_ldr_spin_lock);
...
{
	terminate_tee_session(&isl29023_ctx);
