				    ...
				    const char *buf, size_t count)
{
	...
	if (enable && pdata->active == MPL_STANDBY) {
		...
		if (!ret) {
			...
			printk("mpl3115 set active fails.\n");
...
static void mpl3115_device_init(struct i2c_client *client)
{
	...
	ret = mpl3115_i2c_write(client, MPL3115_CTRL_REG1, val, 1);
	if (ret) {
		pr_err("MPL3115_CTRL_REG1 set fails.\n");
	}

	val[0] = 0x07;
	ret = mpl3115_i2c_write(client, MPL3115_PT_DATA_CFG, val, 1);
	if (ret) {
		pr_err("MPL3115_PT_DATA_CFG set fails.\n");
	}
}
