...
struct ipu_soc ipu_dum;

// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.cm_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.ic_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.idmac_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.dp_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.dc_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.dmfc_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.di_reg[0]);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.di_reg[1]);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.smfc_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.csi_reg[0]);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.csi_reg[1]);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.cpmem_base);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.tpmem_base);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.dc_tmpl_reg);
// DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.vdi_reg);
DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.int_reg_spin_lock);
DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.rdy_reg_spin_lock);
DECLARE_SEC_ELEMENT(ipu_dum, ipu_dum.mutex_lock);
...
{	
	pr_debug("dumping ipu registers ...\n");
...
int32_t ipu_init_channel(struct ipu_soc *ipu, ipu_channel_t channel, ipu_channel_params_t *params)
{
	...
	// pr_debug("ipu pm_runtime_get_sync success.\n");
	/*
	...
	_ipu_get(ipu);
	// pr_debug("ipu get, clk_enable() success.\n");

	mutex_lock(&ipu->mutex_lock);

	// pr_debug("mutex lock success.\n");
	...
	case CSI_MEM3:
		...
		break;
	case CSI_PRP_ENC_MEM: // CSI_PRP_ENC_MEM
				...
				uint32_t u, uint32_t v)
{
	...
	if (_ipu_is_ic_chan(dma_chan) || _ipu_is_vdi_out_chan(dma_chan)) { // true
		if ((width % 16) == 0) /* TBD */
			_ipu_ch_param_set_burst_size(ipu, dma_chan, 16);
		else
	...
	if (_ipu_is_ic_chan(dma_chan) || _ipu_is_irt_chan(dma_chan) ||
		_ipu_is_vdi_out_chan(dma_chan)) { // true
		burst_size = _ipu_ch_param_get_burst_size(ipu, dma_chan);
	...
	if (idma_is_set(ipu, IDMAC_CHA_PRI(dma_chan), dma_chan) &&
		...
		case 15:
			...
			break;
		case 20:
			/* The IDMAC will generate eight AXI bursts upon 
			* the assertion of the DMA request. */
			value = 0x3 << 10;
			break;
		case 21:
			value = 0x3 << 12;
				...
				ipu_buffer_t type, uint32_t bufNum, dma_addr_t phyaddr)
{
	...
	dev_dbg(ipu->dev, "In %s(), dma_chan: %d\n", __func__, dma_chan);
			...
			ipu_buffer_t type, uint32_t bufNum)
{
	...
	// if ((reg & idma_mask(dma_chan)) == 0) { /* buffer is not ready */
	// 	pr_debug("ERROR: BUF%d set ready fails.", bufNum);
	// 	return -EINVAL;
	// }
...
static int optee_ipu_dump_reg(void)
{
	// struct thread_smc_args args;

	// args.a0 = OPTEE_SMC_IPU_DUMP_REG;

	// return optee_fast_smc(&args);
}
...
int32_t ipu_enable_channel(struct ipu_soc *ipu, ipu_channel_t channel)
{
	...
	/* Get input and output dma channels */
	out_dma = channel_2_dma(channel, IPU_OUTPUT_BUFFER);
	in_dma = channel_2_dma(channel, IPU_VIDEO_IN_BUFFER); // NO_DMA

	pr_debug("out_dma channel: %d\n", out_dma);
	pr_debug("in_dma channel: %d\n", in_dma);
	...
	if ((ipu->sec_chan_en[IPU_CHAN_ID(channel)]) &&
		((channel == MEM_PP_MEM) || (channel == MEM_PRP_VF_MEM) ||
		 (channel == MEM_VDI_PRP_VF_MEM))) { // false
		sec_dma = channel_2_dma(channel, IPU_GRAPH_IN_BUFFER);
	...
	if ((ipu->thrd_chan_en[IPU_CHAN_ID(channel)]) &&
		((channel == MEM_PP_MEM) || (channel == MEM_PRP_VF_MEM))) { // false
		...
		ipu_idmac_write(ipu, reg | idma_mask(sec_dma), IDMAC_SEP_ALPHA);
		   ...
		   ((channel == MEM_BG_SYNC) || (channel == MEM_FG_SYNC))) { // false
		thrd_dma = channel_2_dma(channel, IPU_ALPHA_IN_BUFFER);
		reg = ipu_idmac_read(ipu, IDMAC_CHA_EN(thrd_dma));
		ipu_idmac_write(ipu, reg | idma_mask(thrd_dma), IDMAC_CHA_EN(thrd_dma));
		reg = ipu_idmac_read(ipu, IDMAC_SEP_ALPHA);
	...
	if (_ipu_is_ic_chan(in_dma) || _ipu_is_ic_chan(out_dma) ||
		...
		_ipu_is_vdi_out_chan(out_dma)) // true
...
int32_t ipu_enable_csi(struct ipu_soc *ipu, uint32_t csi)
{
	...
	dev_dbg(ipu->dev, "In %s(): enable csi %d\n", __func__, csi);
	...
	// pr_debug("dumping from %s\n", __func__);
	// ipu_dump_registers(ipu);
	/* fast smc to optee core ipu driver for SW IPU reg dumping */
	optee_ipu_dump_reg();
	return 0;
}
...
/* temp stub */
extern void temp_ldr_set_callback_addr(unsigned int addr);
extern int ldr_fiq_callback(unsigned int a0, unsigned int a1);
...
static irqreturn_t ipu_sync_irq_handler(int irq, void *desc)
{
	...
	// pr_debug("In %s(), irq #: %d\n", __func__, irq);
	// pr_debug("ipu_sync_irq_handler addr: 0x%08x.\n", ipu_sync_irq_handler);
	...
	for (i = 0; int_reg[i] != 0; i++) {
		...
		while ((line = ffs(int_stat)) != 0) {
			...
			// 			       ipu->irq_list[line].
			// 			       dev_id);
			/* temp: replace with ldr_fiq_callback() */
			if (ipu->irq_list[line].handler == NULL) {
				goto out;
			}
			temp_ldr_set_callback_addr(ipu->irq_list[line].handler);
			pr_debug("initiating LFC ....\n");
			result |= ldr_fiq_callback(line, ipu->irq_list[line].dev_id);
			pr_debug("LFC result: %d.\n", result);
		    ...
		    uint32_t irq_flags, const char *devname, void *dev_id)
{
	...
	/* register IRQ handler for IPU irq */
...
int sw_probe(unsigned char *param_buf) {
	/* SW ipu will have its own copy of the state var */
	memcpy(&ipu_array[0], param_buf, sizeof(struct ipu_soc));

	/**
	 * The following initialization code will be executed inside the SW.
	 * They should be programed using the SW functions.
	 */

	/* register re-mapping */
	struct ipu_soc *ipu = &ipu_array[0];
	unsigned long ipu_base = dum;
	struct ipu_devtype *devtype = &ipu_type_imx6q.devtype;


	ipu_base = get_device_base(IPU1_BASE);
	// pr_debug("ipu_base: 0x%08x\n");
	...
	memset(&ipu->int_reg_spin_lock, '\0', sizeof(ipu->int_reg_spin_lock));
	memset(&ipu->rdy_reg_spin_lock, '\0', sizeof(ipu->rdy_reg_spin_lock));

	/* mutex initialization */
	memset(&ipu->mutex_lock, '\0', sizeof(ipu->mutex_lock));
	mutex_init(&ipu->mutex_lock);

	/* interrupt handler registration */
	// ldr_sw_irq_register(IPU1_ERR_IRQ, sw_ipu_err_irq_handler, ipu);
	// ldr_sw_irq_register(IPU1_SYNC_IRQ, sw_ipu_sync_irq_handler, ipu);
	ldr_sw_irq_register(IPU1_ERR_IRQ, ipu_err_irq_handler, ipu);
	ldr_sw_irq_register(IPU1_SYNC_IRQ, ipu_sync_irq_handler, ipu);

	return 0;
}

static struct ipu_soc *get_sw_sv(void) {
	return &ipu_array[0];
}
...
int sw_ipu_init_channel(unsigned char *param_buf) {
	...
	struct ipu_soc *ipu = get_sw_sv();
	
	ipu_channel_t channel;
	ipu_channel_params_t *params;

	// pr_debug("sw ipu sv got successfully.\n");

	channel = *(ipu_channel_t *)(param_buf + offset);
	offset += roundup(sizeof(channel), 4);

	params = (ipu_channel_params_t *)(param_buf + offset);
	offset += roundup(sizeof(*params), 4);

	// pr_debug("calling ipu_init_channel() ...\n");

	res = ipu_init_channel(ipu, channel, params);
...
int sw_ipu_uninit_channel(unsigned char *param_buf) {
	...
	ipu_uninit_channel(ipu, channel);
...
int sw_ipu_init_channel_buffer(unsigned char *param_buf) {
	...
	/* param 2 */
	type = *(ipu_buffer_t *)(param_buf + offset);
	offset += roundup(sizeof(ipu_buffer_t), 4);

	/* param 3 */
	pixel_fmt = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	/* param 4 */
	width = *(uint16_t *)(param_buf + offset);
	offset += roundup(sizeof(uint16_t), 4);

	/* param 5 */
	height = *(uint16_t *)(param_buf + offset);
	offset += roundup(sizeof(uint16_t), 4);

	/* param 6 */
	stride = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	/* param 7 */
	rot_mode = *(ipu_rotate_mode_t *)(param_buf + offset);
	offset += roundup(sizeof(ipu_rotate_mode_t), 4);

	/* param 8 */
	phyaddr_0 = *(dma_addr_t *)(param_buf + offset);
	offset += roundup(sizeof(dma_addr_t), 4);

	/* param 9 */
	phyaddr_1 = *(dma_addr_t *)(param_buf + offset);
	offset += roundup(sizeof(dma_addr_t), 4);

	/* param 10 */
	phyaddr_2 = *(dma_addr_t *)(param_buf + offset);
	offset += roundup(sizeof(dma_addr_t), 4);

	/* param 11 */
	u_offset = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	/* param 12 */
	v_offset = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	res = ipu_init_channel_buffer(ipu, channel, type, pixel_fmt,
				width, height, stride, rot_mode,
				phyaddr_0, phyaddr_1, phyaddr_2,
				u_offset, v_offset);
...
int sw_ipu_update_channel_buffer(unsigned char *param_buf) {
	...
	/* param 3 */
	bufNum = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	/* param 4 */
	phyaddr = *(dma_addr_t *)(param_buf + offset);
	offset += roundup(sizeof(dma_addr_t), 4);

	res = ipu_update_channel_buffer(ipu, channel, type, bufNum, phyaddr);
...
int sw_ipu_select_buffer(unsigned char *param_buf) {
	...
	res = ipu_select_buffer(ipu, channel, type, bufNum);
...
int sw_ipu_enable_channel(unsigned char *param_buf) {
	...
	res = ipu_enable_channel(ipu, channel);
...
int sw_ipu_disable_channel(unsigned char *param_buf) {
	...
	/* param 2 */
	wait_for_stop = *(bool *)(param_buf + offset);
	offset += roundup(sizeof(bool), 4);

	res = ipu_disable_channel(ipu, channel, wait_for_stop);
...
int sw_ipu_enable_csi(unsigned char *param_buf) {
	...
	/* param 1 */
	csi = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(uint32_t), 4);

	res = ipu_enable_csi(ipu, csi);
...
int sw_ipu_request_irq(unsigned char *param_buf) {
	...
	/* param 1 */
	irq = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(irq), 4);

	/* param 2 */
	handler = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(handler), 4);

	/* param 3 */
	irq_flags = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(irq_flags), 4);

	/* param 4 */
	devname = (const char *)(param_buf + offset);
	offset += roundup(strlen(devname) + 1, 4);

	/* param 5 */
	dev_id = *(uint32_t *)(param_buf + offset);
	offset += roundup(sizeof(dev_id), 4);

	// pr_debug("channel: %d, handler addr: 0x%08x.\n", irq, handler);
	
	res = ipu_request_irq(ipu, irq, handler, irq_flags, devname, dev_id);
...
int sw_ipu_free_irq(unsigned char *param_buf) {
	...
	ipu_free_irq(ipu, irq, dev_id);

	return res;
}

enum itr_return sw_ipu_sync_irq_handler(struct itr_handler *h) {
	enum itr_return itr_res;
	unsigned int org_id;

	// pr_debug("sw_ipu_sync_irq_handler addr: 0x%08x.\n", sw_ipu_sync_irq_handler);

	org_id = ldr_sw_set_cur_driver_id(IPU_DRIVER_ID);

	itr_res =  ipu_sync_irq_handler(h->it, h->data);

	// pr_debug("end of sw_ipu_sync_irq_handler().\n");
...
enum itr_return sw_ipu_err_irq_handler(struct itr_handler *h) {
	...
	itr_res =  ipu_err_irq_handler(h->it, h->data);
...
{
	/* sw init() */
	ldr_sw_driver_register(IPU_DRIVER_ID, dispatch, get_sw_sv());
	
	dum = 0;

	return 0;
}
module_init(ipu_gen_init);
