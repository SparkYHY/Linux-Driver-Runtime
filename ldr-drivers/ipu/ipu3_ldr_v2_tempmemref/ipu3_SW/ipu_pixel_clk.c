...
extern void *ldr_malloc(size_t size, size_t flags);
		...
		u8 ipu_id, u8 di_id, u8 clk_mux_flags)
{
	...
	mux = ldr_malloc(sizeof(struct clk_di_mux), GFP_KERNEL);
	if (!mux)
		...
		u8 ipu_id, u8 di_id, u8 clk_div_flags)
{
	...
	di_div = ldr_malloc(sizeof(struct clk_di_div), GFP_KERNEL);
	if (!di_div)
		return ERR_PTR(-ENOMEM);
		...
		u8 ipu_id, u8 di_id, u8 clk_gate_flags)
{
	...
	gate = ldr_malloc(sizeof(struct clk_di_gate), GFP_KERNEL);
	if (!gate)
		return ERR_PTR(-ENOMEM);
