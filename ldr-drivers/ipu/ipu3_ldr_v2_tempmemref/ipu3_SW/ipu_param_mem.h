...
/**
 * @param: base	cpmem base address
 * @param: w	word size, 1 byte for most cases
 * @param: bit	field's starting bit inside the word
 * @param: size	field size
 * @param: v	field value
*/
/* i: the field is inside the ith u32 of the word */
#define ipu_ch_param_mod_field_io(base, w, bit, size, v) { \
	...
	reg_offset += i; \
	temp = readl((u32 *)(base) + reg_offset); \
	temp &= ~(mask << off); /* clear the original value */ \
	temp |= (v) << off; \
	writel(temp, (u32 *)(base) + reg_offset); \
	if (((bit)+(size)-1)/32 > i) { \
		reg_offset++; \
