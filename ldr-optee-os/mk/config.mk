...
# When enabled, LDR features will be available.
CFG_LDR ?= y
ifeq ($(CFG_LDR),y)
# When enabled, all the ldr debugging code will be included.
CFG_LDR_DEBUG ?= y

# Extra 1 MB ldr heap size for LDR added in the core/arch/arm/kernel/kern.ld.S
CFG_LDR_HEAP_SIZE ?= 0x100000

# Extra 1MB stack size for LDR, 256 KB for each core
CFG_LDR_STACK_SIZE ?= 0x100000

# Extra 1MB text padding before LDR gate and another 1MB padding
# behind LDR gate so that such region can be configured inside 
# domain 2, always visible to drivers. (long jump will go through
# page table)
CFG_LDR_GATE_PADDING_SIZE ?= 0x100000

# Extract 1MB va padding to set LDR heap and stack SECTION aligned
CFG_LDR_PADDING_SIZE ?= 0x100000
else
CFG_LDR_HEAP_SIZE ?= 0x0
endif