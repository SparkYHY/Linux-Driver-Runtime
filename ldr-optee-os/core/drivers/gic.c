...
static void gic_op_set_it_secure(struct itr_chip *chip, size_t it);
...
static const struct itr_ops gic_ops = {
	.set_it_secure = gic_op_set_it_secure,
	.add = gic_op_add,
...
/* check whether an SPI is secure */
static int gic_is_it_secure(struct gic_data *gd, size_t it) {
	size_t group = 0;
	size_t bit = 0;
	uint32_t sw_it_config;

	if (it < NUM_PPI)
		return -1;
	
	group = it / NUM_INTS_PER_REG;
	bit = it % NUM_INTS_PER_REG;

	sw_it_config = io_read32(gd->gicd_base + GICD_IGROUPR(group));

	return ~(~BIT32(bit) & sw_it_config);
}

static int gic_op_is_it_secure(struct itr_chip *chip, size_t it) {
	struct gic_data *gd = container_of(chip, struct gic_data, chip);

	if (it >= gd->max_it)
		panic();

	return gic_is_it_secure(gd, it);
}

/* set an SPI as secure */
static void gic_set_it_secure(struct gic_data *gd, size_t it) {
	size_t group = 0;
	size_t bit = 0;
	uint32_t sw_it_config;
	...
	io_write32(gd->gicd_base + GICD_IGROUPR(group),
		~BIT32(bit) & sw_it_config);
}
