			       ...
			       paddr_t pa, size_t size)
{
	...
	#ifdef LDR_DEBUG_CORE_MMU
	printk("    carve out pa mem [0x%08lx..0x%08lx) size: 0x%08x\n",
		pa, pa + size, size);
	#endif
}
				      ...
				      size_t nelems)
{
	...
	printk("RAM_NSEC phy memory regions:\n");
	for (; i < num_elems; i++) {
		printk("    Region #%02d [0x%08lx..0x%08lx) size: 0x%08lx\n",
		    i, m[i].addr, m[i].addr + m[i].size, m[i].size);
	}
	#endif
...
TEE_Result cache_op_outer(enum cache_op op, paddr_t pa, size_t len)
{
	...
	bool sec = tee_pbuf_is_sec(pa, len);

	vaddr_t pl310_addr;

	if (sec)
		pl310_addr = pl310_base();
	else
		pl310_addr = pl310_nsbase();
	...
	case DCACHE_AREA_INVALIDATE:
		if (len)
			arm_cl2_invbypa(pl310_addr, pa, pa + len - 1);
		break;
	case DCACHE_CLEAN:
	...
	case DCACHE_AREA_CLEAN:
		if (len)
			arm_cl2_cleanbypa(pl310_addr, pa, pa + len - 1);
		break;
	case DCACHE_CLEAN_INV:
	...
	case DCACHE_AREA_CLEAN_INV:
		if (len)
			arm_cl2_cleaninvbypa(pl310_addr, pa, pa + len - 1);
		break;
	default:
...
bool core_mmu_add_mapping(enum teecore_memtypes type, paddr_t addr, size_t len)
{
	...
	#ifdef LDR_DEBUG_CORE_MMU
	dump_mmap_table(static_memory_map);
	#endif
