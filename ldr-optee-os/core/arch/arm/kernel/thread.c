...
#if defined(CFG_CORE_SANITIZE_KADDRESS) || defined(__clang__)
#define STACK_FIQ_SIZE		3072
#else
#define STACK_FIQ_SIZE		2048
#endif
...
DECLARE_STACK(stack_fiq, CFG_TEE_CORE_NB_CORE, STACK_FIQ_SIZE, static);
...
static void print_stack_limits(void)
{
	...
	for (n = 0; n < CFG_TEE_CORE_NB_CORE; n++) {
		...
		DMSG("fiq [%zu] 0x%" PRIxVA "..0x%" PRIxVA, n, start, end);
	}
	for (n = 0; n < CFG_TEE_CORE_NB_CORE; n++) {
		start = GET_STACK_TOP_SOFT(stack_abt, n);
		end = GET_STACK_BOTTOM(stack_abt, n);
...
extern int if_lfc_stdcall_on(void);
		      ...
		      uint32_t a2, uint32_t a3)
{
	...
	// DMSG("thread args: a0-a3: 0x%08x, 0x%08x, 0x%08x, 0x%08x.",
	//      a0, a1, a2, a3);
	...
	/* reset stack content */
	memset((void *)(thread->stack_va_end - STACK_THREAD_SIZE), 
		NULL, STACK_THREAD_SIZE);
...
static void print_thread_ctx(struct thread_ctx_regs *regs) {
	IMSG("Printing threac_ctx_regs ...");

	IMSG("r0:          \t0x%08x", regs->r0);
	IMSG("r1:          \t0x%08x", regs->r1);
	IMSG("r2:          \t0x%08x", regs->r2);
	IMSG("r3:          \t0x%08x", regs->r3);
	IMSG("r4:          \t0x%08x", regs->r4);
	IMSG("r5:          \t0x%08x", regs->r5);
	IMSG("r6:          \t0x%08x", regs->r6);
	IMSG("r7:          \t0x%08x", regs->r7);
	IMSG("r8:          \t0x%08x", regs->r8);
	IMSG("r9:          \t0x%08x", regs->r9);
	IMSG("r10:         \t0x%08x", regs->r10);
	IMSG("r11:         \t0x%08x", regs->r11);
	IMSG("r12:         \t0x%08x", regs->r12);
	IMSG("usr_sp:      \t0x%08x", regs->usr_sp);
	IMSG("usr_lr:      \t0x%08x", regs->usr_lr);
	IMSG("svc_spsr:    \t0x%08x", regs->svc_spsr);
	IMSG("svc_sp:      \t0x%08x", regs->svc_sp);
	IMSG("svc_lr:      \t0x%08x", regs->svc_lr);
	IMSG("pc:          \t0x%08x", regs->pc);
	IMSG("cpsr:        \t0x%08x", regs->cpsr);
}
...
void thread_alloc_and_run(uint32_t a0, uint32_t a1, uint32_t a2, uint32_t a3)
{
	...
	if (if_lfc_stdcall_on() == 1){
		// IMSG("cpsr: 0x%08x.", read_cpsr());
		// IMSG("thread_std_smc_entry() addr: 0x%08x", thread_std_smc_entry);
		// print_thread_ctx(threads + n);
	}
...
static void set_tmp_stack(struct thread_core_local *l, vaddr_t sp)
{
	...
	thread_set_irq_sp(sp); /* never will be irq mode for armv7 optee */
}
...
static void init_user_kcode(void)
{
	...
	/* only reserve thread_excp_vect visible to user */
	...
	/* the first section(1MB) of the user va is unmapped */
