		   ...
		   const char *ctx __maybe_unused)
{
	...
	dhex_dump(NULL, 0, 0, (void *)ai->va, 0x40);
#endif /*ARM32*/
#ifdef ARM64
...
extern uint32_t nw_callback_wrapper_ret(uint32_t nw_callback_va, 
					struct thread_abort_regs *regs);

int ldr_nw_callback_inv(uint32_t abort_type, struct thread_abort_regs *regs) {
	struct abort_info ai;

	DMSG("inside ldr_nw_callback_inv()");

	set_abort_info(abort_type, regs, &ai);

	/* LDR NW linux callback backward invocation */
	if (ai.abort_type == ABORT_TYPE_PREFETCH && 
	    ai.va > 0x7F000000) {
		DMSG("handling nw backward invocation.");
		return 1;
	} else {
		return 0;
	}
