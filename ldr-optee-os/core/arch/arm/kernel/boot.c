...
#define LDR_DEBUG_BOOT
// #define LDR_DEBUG_DTB
#endif /* CFG_LDR_DEBUG */
...
__weak void init_sec_mon(unsigned long nsec_entry)
{
	...
	#ifdef LDR_DEBUG_BOOT
	size_t core_id = get_core_pos();
	if (core_id == 0) {
		DMSG("Primary CPU nsec_entry addr: 0x%" PRIxPA, nsec_entry);
	} else {
		DMSG("Secondary CPU %zu nsec_entry addr: 0x%" PRIxPA,
		    core_id, nsec_entry);
	}
	#endif
...
#else
	malloc_add_pool(__heap1_start, __heap1_end - __heap1_start);
	ldr_malloc_add_pool(__ldr_heap1_start, __ldr_heap1_end - __ldr_heap1_start, 1);
	ldr_malloc_add_pool(__ldr_heap2_start, __ldr_heap2_end - __ldr_heap2_start, 2);
#endif
...
static struct core_mmu_phys_mem *get_memory(void *fdt, size_t *nelems)
{
	...
	/* dry round to calculate the total count of memory nodes */
	...
	*nelems = n;
	...
	/* actually allocate and initialize all the memory nodes */
	...
	#ifdef LDR_DEBUG_BOOT
	printk("System physical memory regions found from dtb\n");
	printk("    total # of memory nodes: %d\n", *nelems);
	for (n = 0; n < *nelems; n++) {
		printk("    memory node #%02d %-15s 0x%08lx..0x%08lx size: 0x%08lx\n", 
		    n, teecore_memtype_name(mem[n].type), mem[n].addr,
		    mem[n].addr + mem[n].size, mem[n].size);
	}
	#endif
...
static void init_external_dt(unsigned long phys_dt)
{
	...
	#ifdef LDR_DEBUG_DTB
	DMSG("fdt pa : 0x%08lx", virt_to_phys(fdt));
	dhex_dump(NULL, 0, 0, fdt, 0x10);
	#endif /* LDR_DEBUG_DTB */
	...
	/* LDR fdt check */
	if (ldr_check_fdt((vaddr_t)fdt) != TEE_SUCCESS) {
		panic();
	}

	#ifdef LDR_DEBUG_DTB
	ldr_fdt2dts((vaddr_t)fdt);
	#endif /* LDR_DEBUG_DTB */
...
static void discover_nsec_memory(void)
{
	...
	if (fdt) {
		/* get phy mem info from dtb in form of core_mmu_phys_mem */
...
static void init_primary(unsigned long pageable_part, unsigned long nsec_entry)
{
	...
	init_runtime(pageable_part);	/* allocate heap1 */
...
void __weak paged_init_primary(unsigned long fdt)
{
	...
	init_tee_runtime(); /* actually call call_initcalls() */
#endif
#ifdef CFG_VIRTUALIZATION
	...
	if (0 != resolve_exported_symbols((uint32_t)__end, 
		(uint32_t)__csymtab_strings_start)) {
		EMSG("ERROR: LDR initialization fails.");
	}
	...
	/* init ldr gate */
	init_tee_text_pt_desc();

	call_finalcalls();
	IMSG("Primary CPU switching to normal world boot");
}

struct embdata_blob_meta {
	uint32_t offset;
	uint32_t size;
};

struct sym_queue core_sym_queue = 
	TAILQ_HEAD_INITIALIZER(core_sym_queue);

#ifdef CFG_LDR
int 
resolve_exported_symbols(uint32_t emd_addr, uint32_t symstr_addr) {
	// DMSG("emd_addr: 0x%08x, symstr_addr: 0x%08x",
	// 	emd_addr, symstr_addr);

	if (emd_addr == 0 || symstr_addr == 0) {
		EMSG("ERROR: Invalide ldr embdata addr or symstr addr");
		return -1;
	}

	struct boot_embdata *embdata = (struct boot_embdata *)(emd_addr);
	// dhex_dump(NULL, 0, 0, (void *)emd_addr, 0x10);
	// DMSG("LDR embdata size: 0x%08x", embdata->total_len);

	if (embdata->total_len == 0 || embdata->num_blobs != 2 || 
		embdata->hashes_offset % 8 != 0 || 
		embdata->reloc_offset % 8 != 0) {
		EMSG("ERROR: Invalid embedded data blob for LDR.");

		return -1;
	}

	// checking ldr embedded data blob magic
	const uint32_t ldr_magic = 0x4E6F5961;
	uint32_t emb_magic =  *(int *)(emd_addr + embdata->total_len - 4);
	if (ldr_magic != emb_magic) {
		EMSG("ERROR: Invalid ldr embdata magic.");
		return -1;
	}

	/**
	 * For ARMv7 with no PAGER, the embdata only contains the
	 * exported symbols. The structure is shown below.
	 * 
	 * 'struct core_symbol' is inherently 8-byte aligned, thus
	 * no padding is needed.
	 * 
     # +-----------------------------------------------------+
     # | uint32_t: Total length in byte                      |
     # | uint32_t: Number of entries "2"                     |
     # +-----------------------------------------------------+
     # | uint32_t: Offset of symbols from beginning of table |
     # | uint32_t: Length of exported symbols                |
     # +-----------------------------------------------------+
     # | uint32_t: Offset of symstrs from beginning of table |
     # | uint32_t: Length of symstrs                         |
     # +-----------------------------------------------------+
     # | Data of symbols                                     |
     # +-----------------------------------------------------+
     # | Data of symstrs + eventual padding                  |
     # +-----------------------------------------------------+
	*/
	struct embdata_blob_meta *blob_metas = 
		(void *)(emd_addr + sizeof(uint32_t) * 2);

	if (blob_metas[0].size % sizeof(struct core_symbol) != 0) {
		EMSG("Error: Core symbol table not aligned.");
		return -1;
	}

	int sym_num = blob_metas[0].size / sizeof(struct core_symbol);
	if (sym_num < 1) {
		EMSG("ERROR: no sym found");
		return -1;
	}

	struct core_symbol *core_syms = 
		(struct core_symbol *)(emd_addr + blob_metas[0].offset);

	char *symstr = (char *)(emd_addr + blob_metas[1].offset);
	/**
	 * Now we scan the core sym blob and construct the sym queue.
	 * the corresponding string offset inside symstr table should 
	 * be calculated as 
	 * 
	 * (uint32_t)(core_symbol.name - symstr)
	 * 
	 * the relocated symstr table address should be calculated as
	 * 
	 * (char *)(emd_symstr_addr + symstr_offset)
	*/
	/**
	 * There is no need to modify 'core_symbol.vale' because the 
	 * .text section are not relocated and core syms only contain
	 * core function pointers, thus the original function pointer 
	 * address is reserved.
	 * 
	 * TODO: 'core_symbol.vale' needs to be relocated once CORE_ASLR
	 *       is enabled.
	*/
	for (int i = 0; i < sym_num; i++) {
		...
		sym_node->sym.value = core_syms[i].value;

		uint32_t symstr_off = 
			((uint32_t)core_syms[i].name - symstr_addr);
		sym_node->sym.name = (char *)(symstr + symstr_off);

		sym_node->sym.id = 0; /* core symbol */

		// IMSG("SYM name: %s, value: 0x%08x.", 
		// 	sym_node->sym.name, sym_node->sym.value);

		TAILQ_INSERT_TAIL(&core_sym_queue, sym_node, link);
	}

	DMSG("total number of symbols: %d", sym_num);
...
{
	DMSG("Secondary CPU %zu initializing", get_core_pos());
	...
	DMSG("Secondary CPU %zu switching to normal world boot", get_core_pos());
}
...
{	
	#ifdef LDR_DEBUG_BOOT
	if (core_idx == 0) {
		DMSG("set primary CPU nsec_entry addr: 0x%" PRIxPA, entry);
	} else {
		DMSG("set secondary CPU %zu nsec_entry addr: 0x%" PRIxPA,
		    core_idx, entry);
	}
	#endif
