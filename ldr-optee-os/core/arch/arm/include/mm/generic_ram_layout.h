 ...
 *			CORE_MMU_PGDIR_SIZE. An extra mem space of 
 *			CFG_LDR_HEAP_SIZE bytes will be included in addition if
 *			LDR is enabled.
 *
...
/**
 * CFG_LDR_PADDING_SIZE -> extra 1MB padding, optional
 * CFG_LDR_GATE_PADDING_SIZE -> one for [TEE core end, ide gate start]
 * 	another for [ide gate end, ide driver region start] 
*/

/* Add LDR heap va size into tee core va space */
#ifdef CFG_TEE_RAM_VA_SIZE
#define TEE_RAM_VA_SIZE		CFG_TEE_RAM_VA_SIZE\
				+ CFG_LDR_HEAP_SIZE * 2\
				+ CFG_LDR_STACK_SIZE * 2\
				+ CFG_LDR_PADDING_SIZE\
				+ CFG_LDR_GATE_PADDING_SIZE * 2
#else
