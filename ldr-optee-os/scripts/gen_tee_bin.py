...
def get_embdata_bin_origin(elffile):
...
def get_csymtab_bin(elffile):
    global csymtab_bin

    if csymtab_bin is None:
        dump_name = re.compile(r'^\.(csymtab)$')
        csymtab_bin = get_sections(elffile, 0, dump_name)

    return csymtab_bin

def get_csymstr_bin(elffile):
    global csymstr_bin

    if csymstr_bin is None:
        dump_name = re.compile(r'^\.(csymtab_strings)$')
        csymstr_bin = get_sections(elffile, 0, dump_name)

    return csymstr_bin

# It seems like that we have to use the original function name 
# 'get_embdata_bin()' cause a new name like 'get_ldr_embdata_bin()'
# will not get the right runtime result. It is assumed that 
# the original function is used elsewhere from the link.mk.
# The concerned component is the size of the embedded data.
# Using a different function name will have the newly added
# embedded data blob unloaded into the memory.

def get_embdata_bin(elffile):
    global ldr_embdata_bin
    ldr_embdata_magic = 0x4E6F5961 # NoYa
    if ldr_embdata_bin is None:
        csymtab_bin = get_csymtab_bin(elffile)
        csymstr_bin = get_csymstr_bin(elffile)

        num_entries = 2
        csymtab_off = 2 * 4 + num_entries * (2 * 4)
        csymtab_pad = round_up(len(csymtab_bin), 8) - len(csymtab_bin)
        csymstr_off = csymtab_off + len(csymtab_bin) + csymtab_pad
        csymstr_pad = round_up(len(csymstr_bin), 8) - len(csymstr_bin)
        total_len = csymstr_off + len(csymstr_bin) + csymstr_pad + 4

        ldr_embdata_bin = struct.pack('<IIIIII', total_len, num_entries,
                                        csymtab_off, len(csymtab_bin),
                                        csymstr_off, len(csymstr_bin))
        ldr_embdata_bin += csymtab_bin + bytearray(csymtab_pad)
        ldr_embdata_bin += csymstr_bin + bytearray(csymstr_pad)
        ldr_embdata_bin += struct.pack('<I', ldr_embdata_magic)

    return ldr_embdata_bin
