...
static DEFINE_CTX(ied1_malloc_ctx);
static DEFINE_CTX(ied2_malloc_ctx);
...
void *ldr_malloc_ied(size_t size, int ied_num)
{
	void *p;
	uint32_t exceptions;
	
	if (ied_num == 1) {
		exceptions = malloc_lock(&ied1_malloc_ctx);

		p = raw_malloc(0, 0, size, &ied1_malloc_ctx);
		malloc_unlock(&ied1_malloc_ctx, exceptions);
	} else {
		exceptions = malloc_lock(&ied2_malloc_ctx);

		p = raw_malloc(0, 0, size, &ied2_malloc_ctx);
		malloc_unlock(&ied2_malloc_ctx, exceptions);
	}

	return p;
}

void ldr_free_ied(void *ptr, int ied_num)
{
	uint32_t exceptions;
	...
	if (ied_num == 1) {
		...
		raw_free(ptr, &ied1_malloc_ctx, false);
		malloc_unlock(&ied1_malloc_ctx, exceptions);
	} else {
		exceptions = malloc_lock(&ied2_malloc_ctx);

		raw_free(ptr, &ied2_malloc_ctx, false);
		malloc_unlock(&ied2_malloc_ctx, exceptions);
	}	
}
...
/* add a memory region into one ied memory allocator */
int ldr_malloc_add_pool(void *buf, size_t len, int ied_id)
{
	if (ied_id == 1)
		gen_malloc_add_pool(&ied1_malloc_ctx, buf, len);
	else if (ied_id == 2)
		gen_malloc_add_pool(&ied2_malloc_ctx, buf, len);
	else
		return 1;
